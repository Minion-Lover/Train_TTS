...
class ControllableInterface:
    ...
    def read(
        ...
        self.model.set_utterance_embedding(
            path_to_reference_audio="voices/kayla-normal.mp3", embedding=embedding
        )
        ...
        if len(phones) > 1800:
            if language == "German":
                ...
                if self.current_language != "English":
                    self.model.set_phonemizer_language(
                        self.language_id_lookup["English"]
                    )
                    self.current_language = "English"
        ...
        wav = wav.cpu().numpy()
        wav = [
            val for val in wav for _ in (0, 1)
        ]  # doubling the sampling rate for better compatibility (24kHz is not as standard as 48kHz)
